...
HOWTO = """Click on an entry and use
    ...
    Option↑ and Option↓ to increment or decrement by 3650 days"""
...
class Window(Tk):
    ...
    def __init__(self):
        ...
        a = Column(self, label=u"a = b − Δ")
        b = Column(self, label=u"b = (a + c) / 2")
        c = Column(self, label=u"c = b + Δ")
        self.columns = (a, b, c)
    ...
    def reset_columns(self, target_date):
        for column in self.columns:
            column.temporarily_normal_enter()
            column.set_date(target_date)
            column.temporarily_normal_exit()
